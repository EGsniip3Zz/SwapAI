...
export default function SellTool() {
      ...
      <div className="min-h-screen bg-slate-950 pt-20">
        <div className="max-w-2xl mx-auto px-4 py-16 text-center">
          ...
          <p className="text-slate-400 mb-6">
            You need to be registered as a seller to list tools. Sign up as a seller to get started.
          </p>
          <button
            onClick={() => navigate('/signup')}
            className="px-6 py-3 bg-gradient-to-r from-violet-600 to-fuchsia-600 rounded-lg text-white font-semibold"
          >
            Sign Up as Seller
          </button>
        </div>
    ...
    return (
      <div className="min-h-screen bg-slate-950 pt-20">
        <div className="max-w-2xl mx-auto px-4 py-16 text-center">
          ...
          <p className="text-slate-400 mb-6">
            Your tool has been automatically approved and is now live on the marketplace.
            Our team will review it shortly to ensure everything looks good.
          </p>
          <div className="flex items-center justify-center gap-4">
            ...
            <button
              ...
              className="px-6 py-3 bg-slate-800 rounded-lg text-white font-medium"
            >
              List Another Tool
            </button>
          </div>
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.price_type !== 'free' && formData.price_type !== 'contact' && !formData.price) {
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    maxLength={2}
                    className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-lg text-white text-2xl text-center focus:outline-none focus:ring-2 focus:ring-violet-500"
                    placeholder="🤖"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  maxLength={200}
                  className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-lg text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-violet-500"
                  placeholder="A brief one-liner about your tool"
                />
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Describe what your tool does, who it's for, and why it's valuable..."
                />
              </div>
            </div>
          </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {priceTypes.map(type => (
                    <button
                      key={type.value}
                      type="button"
                      onClick={() => setFormData(prev => ({ ...prev, price_type: type.value }))}
                      className={`px-4 py-3 rounded-lg text-sm font-medium transition-all ${
                        formData.price_type === type.value
                          ? 'bg-violet-500/20 border-violet-500 text-white border'
                          : 'bg-slate-800 border-slate-700 text-slate-400 border hover:border-slate-600'
                      }`}
                    >
                      {type.label}
                    </button>
              ...
              {(formData.price_type === 'one-time' || formData.price_type === 'monthly') && (
                <div>
                  <label className="block text-sm font-medium text-slate-300 mb-2">
                    Price (USD) {formData.price_type === 'monthly' && '/ month'} *
                  </label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      min="0"
                      step="0.01"
                      className="w-full pl-8 pr-4 py-3 bg-slate-800 border border-slate-700 rounded-lg text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-violet-500"
                      placeholder="0.00"
                    />
                  </div>
                </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-lg text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-violet-500"
                  placeholder="https://yourwebsite.com/screenshot.png"
                />
                <p className="mt-1 text-xs text-slate-500">Recommended: 16:9 aspect ratio, at least 1280x720px</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
            ...
            <div className="space-y-3">
                ...
                <div key={index} className="flex gap-2">
                  ...
                  {formData.features.length > 1 && (
                    <button
                      ...
                      className="px-3 py-3 bg-slate-800 border border-slate-700 rounded-lg text-slate-400 hover:text-red-400 hover:border-red-500/50 transition-colors"
                    >
                      <X className="w-5 h-5" />
                    </button>
                  )}
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 text-sm text-violet-400 hover:text-violet-300"
              >
              ...
              {formData.tech_stack.map((tech, index) => (
                <div key={index} className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-4 py-3 bg-slate-800 border border-slate-700 rounded-lg text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-violet-500"
                    placeholder="e.g., REST API, Python SDK"
                  />
          ...
          <div className="flex items-center justify-between pt-6 border-t border-slate-800">
            <p className="text-sm text-slate-500">
              Your listing will be automatically approved and live immediately.
            </p>
            <button
              type="submit"
              disabled={loading}
              className="px-8 py-3 bg-gradient-to-r from-violet-600 to-fuchsia-600 hover:from-violet-500 hover:to-fuchsia-500 rounded-lg text-white font-semibold transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
